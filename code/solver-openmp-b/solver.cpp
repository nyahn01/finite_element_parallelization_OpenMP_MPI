...
void femSolver::explicitSolver()
{
    ...
    for (int iter=0; iter<nIter; iter++)
    {
        ...
        #pragma omp parallel
        {
            ...
            #pragma omp for private(elem, M, F, K, TL, i, MTnewL) reduction(+: MTnew[0:nn]) schedule(static,2048)
            //#pragma omp for private(elem, M, F, K, TL, i, MTnewL) reduction(+: MTnew[0:nn]) schedule(dynamic,1)
            //#pragma omp for private(elem, M, F, K, TL, i, MTnewL) reduction(+: MTnew[0:nn]) schedule(guided,2048)
            ...
            // Uncomment one of the following lines to switch between different scheduling strategies.
            #pragma omp for private(pNode, massTmp, MT, Tnew) reduction(+:partialL2error) schedule(static,2048)
            //#pragma omp for private(pNode, massTmp, MT, Tnew) reduction(+:partialL2error) schedule(dynamic,1)
            //#pragma omp for private(pNode, massTmp, MT, Tnew) reduction(+:partialL2error) schedule(guided,2048)
